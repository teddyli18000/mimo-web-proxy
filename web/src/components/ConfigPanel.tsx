...
export function ConfigPanel() {
  ...
  const [defaultModel, setDefaultModel] = useState('mimo-v2.6-pro')
      ...
      <motion.div className={cardClass} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <input type={showKey ? 'text' : 'password'} value={apiKey} onChange={e => setApiKey(e.target.value)} className={inputClass} placeholder="sk-mimo" />
          ...
          <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
            {lang === 'zh'
              ? '填入 config.json 里的 api_key（默认 sk-mimo），保存后用于访问管理接口与 API'
              : 'Fill in the api_key from config.json (default sk-mimo). Used for admin and API access after saving.'}
          </p>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { id: 'mimo-v2.6-pro', label: 'MiMo V2.6 Pro', desc: lang === 'zh' ? '深度推理 · 1T · 默认' : 'Deep reasoning · 1T · Default' },
              { id: 'mimo-v2.6-flash', label: 'MiMo V2.6 Flash', desc: lang === 'zh' ? '日常全模态 · 300B · 快' : 'Daily multimodal · 300B · Fast' },
              { id: 'mimo-v2.6-pro-ultraspeed-studio', label: 'V2.6 UltraSpeed', desc: lang === 'zh' ? '极速响应 · 1T' : 'Ultra-fast · 1T' },
              { id: 'mimo-v2.5', label: 'MiMo V2.5', desc: lang === 'zh' ? '上一代全模态' : 'Legacy multimodal' },
            ].map(m => (
